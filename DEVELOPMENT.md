...
- **관측 지연(lag) 보정**: 마지막 프레임의 `tm`은 실제 현재 시각보다 뒤처져 있을 수 있다(KMA
  ...
  과대평가된다
- 5분 단위로(지금 기준) 최대 60분까지 직선 외삽, 사용자와의 거리가 `max(3km, 1.5×셀 대각선km)`
...
```
IDLE ──(도달 예측 발생)──▶ WATCHING ──(비구름이 5km 밖으로 2주기 연속 확인)──▶ "비가 그쳤어요" 알림 ──▶ IDLE
         │                    │
...
- `WATCHING` 상태에서는 계속 비가 오거나 근처(5km 이내)에 있는 한 추가 알림 없음
