...
WebView 기반 강수 레이더 알림 안드로이드 앱. 백그라운드에서 주기적으로 강수 레이더를 분석해
사용자 위치로 비구름이 다가오는지 예측하고, 알림으로 알려준다. 계산된 비구름 이동 경로는
웹뷰(`https://www.ribs.kr/rain`)에 전달해 지도 위에 그리게 한다.

- 패키지명: `com.goju.ribs.myrainassist`
- 표시 이름: **비올까**
- minSdk 31 / targetSdk·compileSdk 36
- 개인용 사이드로딩 앱 (Play 스토어 배포 대상 아님)
...
2. `RadarApi.fetchFrames(count=6)`로 최근 레이더 프레임 6개 조회
...
> 채널 ID를 새로 발급했다 (`rain_monitor_ongoing` → `_v2` → `_v3`). `IMPORTANCE_MIN`은 상태바
> 아이콘이 아예 안 뜨고, `LOW`는 "무음" 그룹에 묶여 알림창 하단에 깔리는 문제가 있어 최종적으로
> `DEFAULT`(무음 처리)로 정착했다. 다만 상태바 최상단의 작은 아이콘 자체는 제조사/OS 정책에 따라
> 항상 보장되지는 않는다 (알림창을 내리면 항상 정확히 표시됨은 확인됨).
...
- `gridDataBase64` 디코드 시 바이트 길이 = `gridWidth * gridHeight` (셀당 1바이트, row-major)
- 바이트값 `255` = 강수 없음, `2~250` = 강수 echo 존재 (그 외 범위는 실측에서 등장하지 않아 미사용)
- **주의**: 최상위 응답에 `data` 래퍼가 없다. 처음 구현 시 `{"data": {...}}` 형태로 잘못 가정해서
  매 poll 주기가 조용히 실패하던 실제 버그가 있었음 (수정 완료, `RadarApi.kt` 참고)
...
중 median을 사용해 노이즈에 강건하게 만든다. 프레임 간 실제 `tm` 차이를 사용하므로 프레임
누락에도 안전하다.

### 4.5 도달 예측 (`ForecastEngine.kt`)

- blob 위치를 셀당 실제 km(로컬 투영 왜곡 보정, haversine 기반)로 변환해 속도(km/h)·방향(도) 계산
- 5분 단위로 최대 60분까지 직선 외삽, 사용자와의 거리가 `max(3km, 1.5×셀 대각선km)` 이내가
  되는 최초 시점을 도달 시간으로 기록
- 여러 blob 중 최소 도달 시간을 10분 단위로 반올림해 `etaMinutes`로 사용
- 사용자가 있는 정확한 격자 셀이 현재 강수 상태면 `etaMinutes=0` (ACTIVE), 아니면 도달 예측값
  (INCOMING) 또는 `null`(NONE)

### 4.6 알림 상태머신 (`NotificationDedup.kt`)

```
IDLE ──(도달 예측 발생)──▶ WATCHING ──(비구름이 20km 밖으로 2주기 연속 확인)──▶ "비가 그쳤어요" 알림 ──▶ IDLE
         │                    │
...
- `WATCHING` 상태에서는 계속 비가 오거나 근처(20km 이내)에 있는 한 추가 알림 없음
- "그침" 판정은 2주기(~11분) 연속 확인해야 확정 → blob 경계 노이즈로 인한 오탐 방지
- 그친 뒤에는 SharedPreferences를 완전히 초기화해 다음 접근 강수를 처음부터 새로 감지
...
- 실기기(Samsung SM-S947N) + 에뮬레이터(`android-34;google_apis;x86_64`) 양쪽에서 권한 온보딩,
  포그라운드 서비스 상시 실행, 배터리 최적화 예외 정상 동작 확인
...
최초 실행 시 온보딩 화면에서 알림 → 위치 → 백그라운드 위치 → 배터리 최적화 제외 순서로
권한을 요청한다. 모두 허용하면 `https://www.ribs.kr/rain` WebView로 넘어가고, 동시에
`RainMonitorService`가 백그라운드에서 시작된다.
